#!/bin/bash

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Ask user for task ID
read -p "Enter task ID (1, 2, or 3): " TASKID

# Validate input
if [[ ! "$TASKID" =~ ^[1-3]$ ]]; then
    echo "Error: Invalid task ID. Please enter 1, 2, or 3."
    exit 1
fi

LOG_FILE="$SCRIPT_DIR/run_finetuning_task${TASKID}.log"

#exec > "$LOG_FILE" 2>&1

echo cuda_visible_devices: $CUDA_VISIBLE_DEVICES

# Activate environment
source /home/mg873uh/Projects_kb/.venv_fomo/bin/activate

# Change to project directory
cd /home/mg873uh/Projects_kb/baseline-codebase

python src/finetune.py \
    --data_dir=/home/mg873uh/Projects_kb/data/finetuning_preproc \
    --save_dir=/home/mg873uh//Projects_kb/baseline-codebase/_models/finetune/FomoCheckpoints/unet_b \
    --pretrained_weights_path=/home/mg873uh/Projects_kb/baseline-codebase/_models/fomo/unet_b.ckpt \
    --model_name=unet_b \
    --patch_size=96 \
    --taskid=${TASKID} \
    --batch_size=2 \
    --epochs=500 \
    --train_batches_per_epoch=100 \
    --augmentation_preset=basic